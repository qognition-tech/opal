...
import { Blog, getBlog, OgImage } from "@/hooks/wp/use-blog";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!blogResponse || blogResponse.length === 0) {
        return {
            title: "Not Found",
    ...
    const { yoast_head_json } = blog;

    return {
        title: yoast_head_json?.title || blog.title.rendered,
        description: yoast_head_json?.og_description || "",
        openGraph: {
            title: yoast_head_json?.og_title || blog.title.rendered,
            description: yoast_head_json?.og_description || "",
            ...
            url: yoast_head_json?.og_url || blog.link,
            type: "article",
            images:
    ...
    return (
        <div className="flex min-h-screen w-full flex-col gap-4">
            ...
            <div className="flex w-full flex-col gap-4 pt-[132px]">
                <Header title={blog.title.rendered} />
                <BlogBody blog={blog} />
            </div>
            <Footer />
        </div>
...
function BlogBody({ blog }: { blog: Blog }) {
    ...
    const cleanedContent = blog.content.rendered.replace(/ class="[^"]*"/g, "");
