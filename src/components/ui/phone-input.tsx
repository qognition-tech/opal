...
const InputComponent = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(
    ({ className, ...props }, ref) => (
        <Input inputClassName={cn("bg-primary-50 h-10 rounded-s-none rounded-e-lg", className)} {...props} ref={ref} />
    ),
);
        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button
                    type="button"
                    variant="outline"
                    className="flex h-10 gap-1 rounded-s-lg rounded-e-none border-r-0 bg-neutral-50 px-3 focus:z-10"
                    disabled={disabled}
                >
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
    ...
    return (
        <span className="flex h-4 w-6 items-center justify-center overflow-hidden rounded bg-neutral-100 [&_svg]:size-full">
